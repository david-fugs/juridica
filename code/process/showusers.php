...
<html lang="es">
    <head>
		...
		<style>
			...
			.btn-action .fa-solid { margin-right:6px; }

			.btn-edit-custom { border-color: #2c7be5; color: #2c7be5; }
			.btn-edit-custom:hover { background: #2c7be5; color: #fff; }

			.btn-delete-custom { border-color: #e74c3c; color: #e74c3c; }
			.btn-delete-custom:hover { background: #e74c3c; color: #fff; }

			/* Reduce number of distinct colors used globally in this page */
			.card-header.bg-primary { background: #2c3e50 !important; }
			.btn-success { background: #16a085; border-color: #16a085; }
...
<?php
	...
	$query = "SELECT id, usuario, nombre, tipo_usuario FROM usuarios WHERE 
				...
				$('.btn-delete').on('click', function(){
					...
					Swal.fire({
						...
						if(result.isConfirmed){
							$.post('deleteuser.php', { id: id }, function(resp){
								if(resp.success){
									Swal.fire({ icon: 'success', title: 'Eliminado', text: resp.message, timer:1500, showConfirmButton:false }).then(function(){
										location.reload();
									});
								} else {
									Swal.fire({ icon: 'error', title: 'Error', text: resp.message });
