...
if (!empty($abogado)) {
    $where_conditions[] = "usuarios.nombre LIKE ?";
    $params[] = "%$abogado%";
    $types .= 's';
}
...
$headers = ['#', 'Fecha', 'Accionante', 'Documento', 'Radicado', 'Despacho Judicial', 
            'Abogado Asignado', 'Auto Admisorio', 'Días Transcurridos', 
            'Estado Actual', 'Observaciones', 'Estado'];
...
while ($data = $result->fetch_assoc()) {
    ...
    $sheet->setCellValue('B' . $row, date('d/m/Y', strtotime($data['fecha_dem'])));
    $sheet->setCellValue('C' . $row, $data['accionante_dem']);
    $sheet->setCellValue('D' . $row, $data['doc_dem']);
    
    // Radicado como TEXTO para evitar notación científica
    $sheet->setCellValueExplicit('E' . $row, $data['rad_dem'], DataType::TYPE_STRING);
    
    $sheet->setCellValue('F' . $row, $data['desp_judi_dem']);
    ...
    $sheet->setCellValue('J' . $row, $data['est_act_proc_dem']);
    $sheet->setCellValue('K' . $row, $data['obs_dem']);
...
// Generar archivo
$filename = "Demandas_" . date('Y-m-d_His') . ".xlsx";
