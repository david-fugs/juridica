...
$nom_conc = isset($_GET['nom_conc']) ? $_GET['nom_conc'] : '';
$rad_conc = isset($_GET['rad_conc']) ? $_GET['rad_conc'] : '';
...
if (($tipo_usuario == 1 || $tipo_usuario == '1') && !empty($doc_usuario_actual)) {
    $where_conditions[] = "conciliaciones.doc_jur = ?";
...
if (!empty($nom_conc)) {
    $where_conditions[] = "nom_conc LIKE ?";
    $params[] = "%$nom_conc%";
    $types .= 's';
}

if (!empty($rad_conc)) {
    $where_conditions[] = "rad_conc LIKE ?";
    $params[] = "%$rad_conc%";
    $types .= 's';
}
...
while ($data = $result->fetch_assoc()) {
    ...
    $sheet->setCellValue('B' . $row, date('d/m/Y', strtotime($data['fecha_conc'])));
    $sheet->setCellValue('C' . $row, $data['nom_conc'] ?? '');
    $sheet->setCellValue('D' . $row, $data['tipo_conc'] ?? '');
    $sheet->setCellValue('E' . $row, $data['doc_conc'] ?? '');
    
    // Radicado como TEXTO para evitar notación científica
    $sheet->setCellValueExplicit('F' . $row, $data['rad_conc'] ?? '', DataType::TYPE_STRING);
    ...
    $sheet->setCellValue('J' . $row, $data['obs_conc'] ?? '');
...
// Generar archivo
$filename = "Conciliaciones_" . date('Y-m-d_His') . ".xlsx";
