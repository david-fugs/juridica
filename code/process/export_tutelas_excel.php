...
$rad_tut = isset($_GET['rad_tut']) ? $_GET['rad_tut'] : '';
...
if (($tipo_usuario == 1 || $tipo_usuario == '1') && !empty($doc_usuario_actual)) {
    $where_conditions[] = "tutelas.doc_jur = ?";
...
if (!empty($nom_tut)) {
    $where_conditions[] = "nom_tut LIKE ?";
    $params[] = "%$nom_tut%";
    $types .= 's';
}

if (!empty($rad_tut)) {
    $where_conditions[] = "rad_tut LIKE ?";
    $params[] = "%$rad_tut%";
    $types .= 's';
}
...
$sheet->setTitle('Tutelas');

// Configurar encabezados
$headers = ['#', 'Fecha', 'Accionante', 'Tipo', 'Documento', 'Radicado', 
            'Estado Tutela', 'Abogado Asignado', 'Auto Admisorio', 'Días Transcurridos', 
...
while ($data = $result->fetch_assoc()) {
    ...
    $sheet->setCellValue('B' . $row, date('d/m/Y', strtotime($data['fecha_tut'])));
    $sheet->setCellValue('C' . $row, $data['nom_tut'] ?? '');
    $sheet->setCellValue('D' . $row, $data['tipo_tut'] ?? '');
    $sheet->setCellValue('E' . $row, $data['doc_tut'] ?? '');
    
    // Radicado como TEXTO para evitar notación científica
    $sheet->setCellValueExplicit('F' . $row, $data['rad_tut'] ?? '', DataType::TYPE_STRING);
    
    $sheet->setCellValue('G' . $row, $data['estado_tut'] ?? '');
    $sheet->setCellValue('H' . $row, $data['nom_jur'] ?? '');
    $sheet->setCellValue('I' . $row, 
        !empty($data['auto_admisorio']) && $data['auto_admisorio'] !== '0000-00-00' 
        ? date('d/m/Y', strtotime($data['auto_admisorio'])) 
        : '—');
    $sheet->setCellValue('J' . $row, is_null($days_passed) ? '—' : $days_passed);
    $sheet->setCellValue('K' . $row, $data['obs_tut'] ?? '');
    $sheet->setCellValue('L' . $row, $estado_texto);
...
// Generar archivo
$filename = "Tutelas_" . date('Y-m-d_His') . ".xlsx";
